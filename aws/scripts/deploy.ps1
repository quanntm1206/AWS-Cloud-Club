...
aws cloudformation deploy --stack-name $stack --template-file aws/cloudformation/tabular-inference.yml `
    ...
    --parameter-overrides ProjectId=$ProjectId Owner=$Owner ExpiresAt=$ExpiresAt EnablePublicApi=$public `
...
if ($LASTEXITCODE -ne 0) { throw "CloudFormation output lookup failed: exit $LASTEXITCODE" }
...
if ($LASTEXITCODE -ne 0) { throw "Artifact upload failed: exit $LASTEXITCODE" }
